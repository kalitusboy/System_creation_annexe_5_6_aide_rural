...
/// <summary>
/// تحويل عدد صحيح إلى كلمات بالفرنسية (نفس منطق ملف HTML تماماً).
/// </summary>
public static class NumberToFrenchWords
{
    ...
    public static string Convert(long num)
    {
        ...
        if (million > 0)
            sb.Append(million == 1 ? "UN MILLION" : Below1000(million) + " MILLIONS").Append(' ');
        if (thousand > 0)
            sb.Append(thousand == 1 ? "MILLE" : Below1000(thousand) + " MILLE").Append(' ');
        if (rest > 0)
    ...
    public static string Percent(int pct) => Convert(pct) + " POUR CENT";

    /// <summary>
    /// تنسيق المبلغ بصيغة الجزائر: 420.000,00 → في القالب يكتبونه 420.000.00
    /// </summary>
