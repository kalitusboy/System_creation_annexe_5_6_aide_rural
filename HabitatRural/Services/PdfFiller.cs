...
/// <summary>
/// يفتح قالب PDF الرسمي ويحقن البيانات داخل الحقول التفاعلية (AcroForm)
/// ثم يحفظ النسخة الجديدة. هذا يضمن إخراجاً بالتنسيق الرسمي 100%.
/// </summary>
public class PdfFiller
{
    /// <summary>
    /// قائمة الرسائل لتشخيص الحقول (تظهر في لوحة "سجل العمليات").
    /// </summary>
    ...
    /// <summary>
    /// تعبئة قالب الملحق 05.
    /// </summary>
    public byte[] FillAnnexe05(string templatePath, AppSettings settings,
                               AnnexeRequest req, AnnexeLogic logic)
    {
        ...
        // التواريخ الاختيارية تُترك فارغة إن لم تُملأ
        SetText(form, "date_demande",
            req.DateDemande?.ToString("dd/MM/yyyy") ?? "");
    ...
    /// <summary>
    /// تعبئة قالب الملحق 06.
    /// </summary>
    public byte[] FillAnnexe06(string templatePath, AppSettings settings,
                               AnnexeRequest req, AnnexeLogic logic)
    {
        using var doc = PdfReader.Open(templatePath, PdfDocumentOpenMode.Modify);
        var form = doc.AcroForm;
    ...
    /// <summary>
    /// إجبار قارئات PDF (Adobe/Foxit/...) على إعادة رسم الحقول النصية بعد التعبئة.
    /// </summary>
    ...
    /// <summary>
    /// حقول متكررة بنفس الاسم (Wilaya/Daira/Commune قد تظهر مرتين في القالب).
    /// </summary>
    ...
    /// <summary>
    /// عرض أسماء الحقول الموجودة في القالب (للتشخيص).
    /// </summary>
