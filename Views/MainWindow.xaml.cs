...
public partial class MainWindow : Window
{
    ...
    private void RefreshBeneficiairesDropdown()
    {
        ...
        var list = new ObservableCollection<Beneficiaire>(DatabaseService.Instance.GetAllBeneficiaires());
        CbExistingBeneficiaires.ItemsSource = list;
        if (prev != null) CbExistingBeneficiaires.SelectedItem = list.FirstOrDefault(x => x.Id == prev.Id);
    }
    ...
    private void BtnDeleteBeneficiaire_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (MessageBox.Show($"هل تريد حذف المستفيد:\n{b.NomPrenom}؟", "تأكيد الحذف",
                MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
        {
            DatabaseService.Instance.DeleteBeneficiaire(b.Id);
            RefreshBeneficiairesDropdown();
    ...
    private void TxtCode_TextChanged(object sender, TextChangedEventArgs e) { UpdateLast5(); RecomputeLogic(); }
    ...
    private TrancheType GetTranche() => RbT1.IsChecked == true ? TrancheType.T1Only : (RbT2.IsChecked == true ? TrancheType.T2Only : TrancheType.Both);
    private BuildingType GetBuilding() => RbTalia.IsChecked == true ? BuildingType.Talia : BuildingType.Ardhi;

    private void RecomputeLogic()
    {
        if (TxtLogicSummary == null) return;
        var req = new AnnexeRequest { Beneficiaire = BuildBeneficiaireFromForm(), Tranche = GetTranche(), Building = GetBuilding() };
        var logic = LogicService.Compute(SettingsService.Instance.Current, req);
        var trStr = req.Tranche switch { TrancheType.T1Only => "الشطر الأول", TrancheType.T2Only => "الشطر الثاني", _ => "الشطرين معاً" };
        var bdStr = req.Building == BuildingType.Talia ? "تعلية" : "أرضي";
        TxtLogicSummary.Text = $"⚙ المنطق النشط: {trStr} | {bdStr} | المبلغ: {logic.AmountNum} دج | النسب: {logic.Pct1Num} / {logic.Pct2Num}";
    }

    private void TabAnnexes_SelectionChanged(object sender, SelectionChangedEventArgs e) => UpdateTemplateLabel();
    private void UpdateTemplateLabel() => RunTemplateName.Text = TabAnnexes.SelectedIndex == 0 ? "Annexe_05_Officiel.pdf" : "Annexe_06_Officiel.pdf";
    ...
    private void BtnBeneficiaires_Click(object sender, RoutedEventArgs e) => CbExistingBeneficiaires.IsDropDownOpen = true;

    private bool ValidateShared() => !(string.IsNullOrWhiteSpace(TxtCode.Text) || string.IsNullOrWhiteSpace(TxtNomPrenom.Text) ||
        string.IsNullOrWhiteSpace(TxtAdresse.Text) || string.IsNullOrWhiteSpace(TxtCcp.Text) || DpDateDecision.SelectedDate == null);

    private bool ValidateAnnexe06() => !(string.IsNullOrWhiteSpace(CbSubdiv.Text) || string.IsNullOrWhiteSpace(TxtPermisNum.Text) || DpDatePermis.SelectedDate == null);
    ...
    private void BtnShowFields_Click(object sender, RoutedEventArgs e)
    {
        ...
        catch (Exception ex) { _logs.Add("❌ " + ex.Message); ShowToast("❌ خطأ: " + ex.Message, ToastType.Error); }
    }
    ...
    private void BtnGenerate_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            byte[] pdfBytes = isA5 ? _filler.FillAnnexe05(template, settings, req, logic) : _filler.FillAnnexe06(template, settings, req, logic);
            foreach (var l in _filler.Logs) _logs.Add(l);

            var archivesFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "HabitatRural_Archives");
            Directory.CreateDirectory(archivesFolder);
            string trancheCode = req.Tranche switch { TrancheType.T1Only => "T1", TrancheType.T2Only => "T2", _ => "Both" };
            string fileName = $"{(isA5 ? "Annexe05" : "Annexe06")}_{req.Beneficiaire.Code}_{req.Beneficiaire.NomPrenom.Replace(" ", "_")}_{trancheCode}_{DateTime.Now:yyyyMMdd_HHmmss}.pdf";
            var archivePath = Path.Combine(archivesFolder, SafeFileName(fileName));
            File.WriteAllBytes(archivePath, pdfBytes);
            _logs.Add($"💾 نسخة محفوظة في الأرشيف: {archivePath}");

            var archiveEntry = new ArchiveEntry
            {
                BeneficiaireId = req.Beneficiaire.Id,
                TypeAnnexe = isA5 ? "05" : "06",
                Tranche = trancheCode,
                DateGeneration = DateTime.Now,
                CheminFichier = archivePath
            };
            if (req.Beneficiaire.Id > 0)
                DatabaseService.Instance.SaveArchive(archiveEntry);
            else
                _logs.Add("⚠ لم يتم حفظ الأرشيف لأن المستفيد غير مخزّن في قاعدة البيانات");

            var dlg = new SaveFileDialog
            {
                Title = "حفظ ملف PDF",
                Filter = "PDF Files (*.pdf)|*.pdf",
                FileName = $"{(isA5 ? "Annexe_05_" : "Annexe_06_")}{SafeFileName(req.Beneficiaire.NomPrenom)}.pdf"
            };
            if (dlg.ShowDialog() == true)
            {
                ...
                if (MessageBox.Show("هل تريد فتح الملف الآن؟", "تم", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.Yes)
                    System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo { FileName = dlg.FileName, UseShellExecute = true });
            }
        }
        catch (Exception ex) { _logs.Add("❌ خطأ: " + ex.Message); ShowToast("❌ خطأ: " + ex.Message, ToastType.Error); }
    }
    ...
    {
        var fileName = TabAnnexes.SelectedIndex == 0 ? "Annexe_05_Officiel.pdf" : "Annexe_06_Officiel.pdf";
        var path = Path.Combine(AppContext.BaseDirectory, "Assets", "Templates", fileName);
        if (!File.Exists(path))
            throw new FileNotFoundException($"القالب الرسمي غير موجود: {path}\nيرجى التأكد من نسخ ملف {fileName} داخل مجلد Assets/Templates.");
        return path;
    }
}
