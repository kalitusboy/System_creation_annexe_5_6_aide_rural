...
public partial class SettingsWindow : Window
{
    ...
    private void BtnAddSubdiv_Click(object sender, RoutedEventArgs e) => _subdivs.Add(new StringBox { Value = "" });
    private void BtnRemoveSubdiv_Click(object sender, RoutedEventArgs e) { if (sender is Button btn && btn.Tag is StringBox sb) _subdivs.Remove(sb); }

    private void BtnSave_Click(object sender, RoutedEventArgs e)
    {
        if (!int.TryParse(TxtPctT1.Text, out var pct1) || pct1 < 0 || pct1 > 100) { MessageBox.Show("نسبة الشطر الأول غير صالحة.", "خطأ"); return; }
        if (!int.TryParse(TxtPctT2.Text, out var pct2) || pct2 < 0 || pct2 > 100) { MessageBox.Show("نسبة الشطر الثاني غير صالحة.", "خطأ"); return; }
        if (pct1 + pct2 != 100) { MessageBox.Show($"مجموع نسب الأشطر يجب أن يساوي 100% (حالياً {pct1 + pct2}%).", "خطأ"); return; }
        if (!decimal.TryParse(TxtTotalAmount.Text, out var total) || total <= 0) { MessageBox.Show("المبلغ الإجمالي غير صالح.", "خطأ"); return; }
